...
func TestSpeciesEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set STARWARSDATABANK_TEST_SPECIES_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		speciesRef01DataDt0LoadResult := core.ToMapAny(speciesRef01DataDt0Loaded)
...
func speciesBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("STARWARSDATABANK_TEST_SPECIES_ENTID")
	...
	idmapResolved := core.ToMapAny(env["STARWARSDATABANK_TEST_SPECIES_ENTID"])
