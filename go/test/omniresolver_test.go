...
import (
	...
	omni "github.com/voxgig-sdk/star-wars-databank-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.StarWarsDatabankError); is {
				out := map[string]any{
					"name":    "StarWarsDatabankError",
...
func omniCtx(arg any, client *sdk.StarWarsDatabankSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.StarWarsDatabankSDK); is {
			client = live
