...
func TestVehicleEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set STARWARSDATABANK_TEST_VEHICLE_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		vehicleRef01DataDt0LoadResult := core.ToMapAny(vehicleRef01DataDt0Loaded)
...
func vehicleBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("STARWARSDATABANK_TEST_VEHICLE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["STARWARSDATABANK_TEST_VEHICLE_ENTID"])
