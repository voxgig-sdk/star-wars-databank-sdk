...
func TestDroidEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Droid(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Droid(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set STAR_WARS_DATABANK_TEST_DROID_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		droidRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.droid")))
		...
		droidRef01DataDt0LoadResult := core.ToMapAny(entityData(droidRef01DataDt0Loaded))
...
func droidBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("STAR_WARS_DATABANK_TEST_DROID_ENTID")
	...
	idmapResolved := core.ToMapAny(env["STAR_WARS_DATABANK_TEST_DROID_ENTID"])
