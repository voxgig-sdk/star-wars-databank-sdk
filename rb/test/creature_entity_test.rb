...
class CreatureEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = StarWarsDatabankSDK.test(seed, nil)
    seen = base.Creature(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Creature(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = creature_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "creature." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    creature_ref01_data = nil
    if creature_ref01_data_raw.length > 0
      creature_ref01_data = Helpers.to_map(creature_ref01_data_raw[0][1])
    end

    # LIST
    creature_ref01_ent = client.Creature(nil)
    creature_ref01_match = {}

    creature_ref01_list_result = creature_ref01_ent.list(creature_ref01_match, nil)
    assert creature_ref01_list_result.is_a?(Array)
...
def creature_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "creature", "CreatureTestData.json")
