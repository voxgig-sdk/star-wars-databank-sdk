...
module StarWarsDatabankOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if StarWarsDatabankOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = StarWarsDatabankOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| StarWarsDatabankOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || StarWarsDatabankOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = StarWarsDatabankSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = StarWarsDatabankError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { StarWarsDatabankOmni.errify(err) }
