# StarWarsDatabank SDK Creature entity
...
class CreatureEntity
  ...
  def make
    opts = @_entopts.dup
    CreatureEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = StarWarsDatabankHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Creature, Hash] the current Creature data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = StarWarsDatabankHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Creature fields)
  def match_get
  ...
  #
  # @param reqmatch [CreatureLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Creature.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Creature, Hash] the loaded Creature; raises StarWarsDatabankError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = StarWarsDatabankHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [CreatureListMatch, Hash, nil] match filter (any subset of
  #   Creature fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Creature>, Array] the matching Creature items; raises StarWarsDatabankError on failure
