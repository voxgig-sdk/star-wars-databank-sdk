...
module StarWarsDatabankUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = StarWarsDatabankResponse.new({})
    ...
    if fetch_err
      ...
      response = StarWarsDatabankResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = StarWarsDatabankResponse.new(fetched)
    else
