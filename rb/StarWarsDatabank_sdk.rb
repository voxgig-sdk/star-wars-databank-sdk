...
class StarWarsDatabankSDK
    ...
    if feature_opts
      ...
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = StarWarsDatabankHelpers.to_map(feature_opts[fname])
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = StarWarsDatabankError.new(
        "graphql_error", "StarWarsDatabankSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.Creature.list / client.Creature.load({ "id" => ... })
  ...
  # Canonical facade: client.Droid.list / client.Droid.load({ "id" => ... })
  def Droid(data = nil)
