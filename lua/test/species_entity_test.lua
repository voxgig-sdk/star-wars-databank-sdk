...
describe("SpeciesEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set STARWARSDATABANK_TEST_SPECIES_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    local species_ref01_data_dt0_load_result = helpers.to_map(species_ref01_data_dt0_loaded)
...
function species_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("STARWARSDATABANK_TEST_SPECIES_ENTID")
