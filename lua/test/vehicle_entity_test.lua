...
describe("VehicleEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set STARWARSDATABANK_TEST_VEHICLE_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    local vehicle_ref01_data_dt0_load_result = helpers.to_map(vehicle_ref01_data_dt0_loaded)
...
function vehicle_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("STARWARSDATABANK_TEST_VEHICLE_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["STARWARSDATABANK_TEST_VEHICLE_ENTID"])
