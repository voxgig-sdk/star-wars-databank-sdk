...
local CreatureEntity = {}
CreatureEntity.__index = CreatureEntity


function CreatureEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CreatureEntity)
  self._name = "creature"
  self._client = client
...
function CreatureEntity:get_name()
  return self._name
end


function CreatureEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CreatureEntity.new(self._client, opts)
end
...
function CreatureEntity:mark_deleted()
  self._deleted = true
end


function CreatureEntity:deleted()
  return true == self._deleted
end


function CreatureEntity:data_set(args)
  if args ~= nil then
...
function CreatureEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CreatureEntity:match_set(args)
  if args ~= nil then
...
function CreatureEntity:match_get()
...
function CreatureEntity:stream(action, args, callopts)
...
---@param reqmatch CreatureLoadMatch
---@param ctrl? table
---@return Creature
---@return string? err
function CreatureEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch CreatureListMatch
---@param ctrl? table
---@return Creature[]
---@return string? err
function CreatureEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function CreatureEntity:_run_op(ctx, post_done)
  local utility = self._utility
