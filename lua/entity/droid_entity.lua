-- StarWarsDatabank SDK Droid entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DroidEntity = {}
DroidEntity.__index = DroidEntity


function DroidEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DroidEntity)
  self._name = "droid"
  self._client = client
...
function DroidEntity:get_name()
  return self._name
end


function DroidEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DroidEntity.new(self._client, opts)
end
...
function DroidEntity:mark_deleted()
  self._deleted = true
end


function DroidEntity:deleted()
  return true == self._deleted
end


function DroidEntity:data_set(args)
  if args ~= nil then
...
function DroidEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DroidEntity:match_set(args)
  if args ~= nil then
...
function DroidEntity:match_get()
...
function DroidEntity:stream(action, args, callopts)
...
---@param reqmatch DroidLoadMatch
---@param ctrl? table
---@return Droid
---@return string? err
function DroidEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch DroidListMatch
---@param ctrl? table
---@return Droid[]
---@return string? err
function DroidEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function DroidEntity:_run_op(ctx, post_done)
  local utility = self._utility
