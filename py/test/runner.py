# StarWarsDatabank SDK test runner
...
class StarWarsDatabankTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            StarWarsDatabankTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = StarWarsDatabankTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = StarWarsDatabankTestRunner.getenv("STAR_WARS_DATABANK_TEST_LIVE")
        override = StarWarsDatabankTestRunner.getenv("STAR_WARS_DATABANK_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = StarWarsDatabankTestRunner.getenv(key)
        ...
        explain = StarWarsDatabankTestRunner.getenv("STAR_WARS_DATABANK_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if StarWarsDatabankTestRunner._test_control is not None:
            return StarWarsDatabankTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                StarWarsDatabankTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            StarWarsDatabankTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = StarWarsDatabankTestRunner.load_test_control()
...
def load_env_local():
    StarWarsDatabankTestRunner.load_env_local()


def env_override(m):
    return StarWarsDatabankTestRunner.env_override(m)


def entity_data(v):
    return StarWarsDatabankTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return StarWarsDatabankTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return StarWarsDatabankTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return StarWarsDatabankTestRunner.load_test_control()


def live_client_options():
    return StarWarsDatabankTestRunner.live_client_options()


def live_delay_ms():
    return StarWarsDatabankTestRunner.live_delay_ms()
