...
class TestDroidEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Droid(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Droid(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set STAR_WARS_DATABANK_TEST_DROID_ENTID JSON to run live")
        client = setup["client"]
        ...
        droid_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(droid_ref01_data_dt0_loaded))
...
def _droid_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("STAR_WARS_DATABANK_TEST_DROID_ENTID"))
