...
from starwarsdatabank_sdk.starwarsdatabank_types import (
    Creature,
    CreatureLoadMatch,
    CreatureListMatch,
)
...
class CreatureEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CreatureEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Creature:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Creature().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Creature]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Creature().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
