...
[MCP](https://modelcontextprotocol.io) server exposing the StarWarsDatabank SDK as
two agent tools — `star-wars-databank_list` and `star-wars-databank_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/star-wars-databank-mcp)
make build

# 2. Provide credentials via the environment
export STAR_WARS_DATABANK_APIKEY=sk_live_xxx
...
```jsonc
// star-wars-databank_list: first page of records
{ "entity": "character" }
{ "entity": "character", "query": { } }

// star-wars-databank_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/star-wars-databank-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user star-wars-databank \
     -- "$PWD"/dist/*/star-wars-databank-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `star-wars-databank_list` and `star-wars-databank_load` tools now appear
   in new sessions. Ask the agent to *"list character using star-wars-databank"*
   and it calls `star-wars-databank_list` with `{"entity":"character"}`.
...
```sh
export STAR_WARS_DATABANK_APIKEY=sk_live_xxx            # API key
export STAR_WARS_DATABANK_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `star-wars-databank_list` tool
...
### Call the `star-wars-databank_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `star-wars-databank_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `star-wars-databank_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `STAR_WARS_DATABANK_APIKEY` | API key sent with every request. |
| `STAR_WARS_DATABANK_BASE` | Optional override of the API base URL. |
