...
type Args struct {
	Entity string         `json:"entity" jsonschema:"character | creature | droid | location | organization | species | vehicle"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.StarWarsDatabankSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "star-wars-databank_list",
		Description: "List records from StarWarsDatabank. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "star-wars-databank_load",
		Description: "Load a single record from StarWarsDatabank. " +
...
func runOp(client *sdk.StarWarsDatabankSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.StarWarsDatabankSDK, name string) (sdk.StarWarsDatabankEntity, error) {
	switch strings.ToLower(name) {
	case "character":
		return client.Character(nil), nil
	case "creature":
		return client.Creature(nil), nil
	case "droid":
		return client.Droid(nil), nil
	case "location":
