...
// StarWarsDatabank SDK feature test
//
...
class FtCtrl extends StarWarsDatabankControl
{
...
class FtClient
{
    ...
    private StarWarsDatabankUtility $utility;

    public function __construct(StarWarsDatabankUtility $utility, array $options)
    {
    ...
    public function get_utility(): StarWarsDatabankUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (StarWarsDatabankContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public StarWarsDatabankContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (StarWarsDatabankContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = StarWarsDatabankConfig::make_feature($name);
    ...
    private static function buildUrl(StarWarsDatabankSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof StarWarsDatabankError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new StarWarsDatabankResponse($fetched);
        }
    ...
    private function fail(StarWarsDatabankContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (StarWarsDatabankContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
