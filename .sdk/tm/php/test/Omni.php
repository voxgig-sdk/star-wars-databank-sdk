...
//    only - a typed StarWarsDatabankContext there would make every ctx
...
final class StarWarsDatabankOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new StarWarsDatabankContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new StarWarsDatabankSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new StarWarsDatabankResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new StarWarsDatabankError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new StarWarsDatabankResponse($clean['response']);
    ...
    public static function err_from_map($m): ?StarWarsDatabankError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new StarWarsDatabankError(is_string($code) ? $code : '', $msg);
    }
