...
describe('CreatureDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new StarWarsDatabankSDK({
  ...
  test('direct-load-creature', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-creature', setup.live)) return
  ...
  test('direct-list-creature', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-creature', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['STAR_WARS_DATABANK_TEST_CREATURE_ENTID']
