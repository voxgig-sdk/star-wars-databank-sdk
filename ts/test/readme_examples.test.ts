...
//     (\`new StarWarsDatabankSDK(...)\` / \`StarWarsDatabankSDK.test(...)\` become
//     \`StarWarsDatabankSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'StarWarsDatabankSDK'
...
const TEST_SEED = {"entity":{"character":{"test01":{"id":"test01"}},"creature":{"test01":{"id":"test01"}},"droid":{"test01":{"id":"test01"}},"location":{"test01":{"id":"test01"}},"organization":{"test01":{"id":"test01"}},"species":{"test01":{"id":"test01"}},"vehicle":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new StarWarsDatabankSDK(...)\` and any
// \`StarWarsDatabankSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+StarWarsDatabankSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/StarWarsDatabankSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
