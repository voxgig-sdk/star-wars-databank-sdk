...
// StarWarsDatabank SDK retry feature
...
class StarWarsDatabankRetryFeature extends StarWarsDatabankBaseFeature
{
    ...
    public function init(StarWarsDatabankContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (StarWarsDatabankContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(StarWarsDatabankContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(StarWarsDatabankContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
