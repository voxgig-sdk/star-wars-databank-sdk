...
// StarWarsDatabank SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class StarWarsDatabankTestFeature extends StarWarsDatabankBaseFeature
{
    ...
    public function init(StarWarsDatabankContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (StarWarsDatabankContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = StarWarsDatabankParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (StarWarsDatabankContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(StarWarsDatabankContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = StarWarsDatabankParam::call($ctx, $k);
