...
// StarWarsDatabank SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class StarWarsDatabankMakePoint
{
    public static function call(StarWarsDatabankContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof StarWarsDatabankError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = StarWarsDatabankHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = StarWarsDatabankHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
