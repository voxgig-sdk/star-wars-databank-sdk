...
class DroidEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = StarWarsDatabankSDK::test(null, null);
        $ent = $testsdk->Droid(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Droid(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Droid(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = droid_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "droid." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $droid_ref01_data = null;
        if (count($droid_ref01_data_raw) > 0) {
...
function droid_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/droid/DroidTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["droid01", "droid02", "droid03"] as $k) {
