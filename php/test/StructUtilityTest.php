...
// StarWarsDatabank SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = StarWarsDatabankOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(StarWarsDatabankOmni::structfix($now, Struct::undef()))
                        !== json_encode(StarWarsDatabankOmni::structfix($val, Struct::undef()))) {
