# star-wars-databank-cli

boru-driven command-line client **and** interactive REPL for the StarWarsDatabank
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/star-wars-databank-cli)
make build

# 2. See usage (words, entities, env vars)
./star-wars-databank-cli --help

# 3. Provide credentials once, via the environment
export STAR_WARS_DATABANK_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./star-wars-databank-cli list character
./star-wars-databank-cli load 1 character            # {id:1} shorthand
./star-wars-databank-cli load '{id:1}' character       # explicit match map
./star-wars-databank-cli list creature

# 5. Override the API base URL for a single call
STAR_WARS_DATABANK_BASE=https://api.example.com ./star-wars-databank-cli list character

# 6. No arguments -> interactive REPL
./star-wars-databank-cli
star-wars-databank> list character
star-wars-databank> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/star-wars-databank-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/star-wars-databank-cli list character
   ```
...
```sh
./star-wars-databank-cli load 1 character          # scalar shorthand for {id:1}
./star-wars-databank-cli load '{id:1}' character     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `star-wars-databank>`). Each line is
evaluated as its own boru expression:

```text
$ ./star-wars-databank-cli
star-wars-databank> list character
star-wars-databank> /help
star-wars-databank> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/star-wars-databank-cli`. |
...
(`./star-wars-databank-cli <expr>`) and interactively (the REPL), and expressions compose the
